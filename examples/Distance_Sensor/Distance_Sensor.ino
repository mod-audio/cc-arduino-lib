/*
	...
	the potentiometer is used to control the sensitivity of the sensor by using a weighted average.  (based on notes by David Schultz at http://home.earthlink.net/~david.schultz/rnd/2002/KalmanApogee.pdf)
	...
	there is also the possibility to use the sensor without an extra potentiometer. 
	this can be done by replacing the analogRead() function by any number between 0 and 1024.(this number will set your sensitivity)
...
//use this line to invert the result (0 sets the largest distance as largest value of the actuator, 1 sets the largest distance as smallest value of the actuator)
#define inv 1
...
{ 
  ...
  //potentiometer for setting sensitivity of  the sensor (if no potentiometer is connected enter a value between 0 and 1023)
   PotValue = 512; //(((analogRead(A0))/1023.0));
...
//these boundaries are set by both the sensor and Control Chain 
 void readminmax(){
  MINDISTANCE = 20; //map((analogRead(A1)), 0, 1023, 5, 20);
  MAXDISTANCE = 50; //map(analogRead(A2), 0, 1023, 20, 65);
  }
